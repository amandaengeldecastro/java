...
public class Main {
    ...
    private static void removeNumber() {
        ...
        System.out.println("Informe a coluna da posição que deseja remover");
        var col = runUntilGetValidNumber(0, 8);
        System.out.println("Informe a linha da posição que deseja remover");
        var row = runUntilGetValidNumber(0, 8);
    ...
    private static String[] generateEmptyBoardArgs() {
        String[] result = new String[81];
        int index = 0;
        for (int i = 0; i < BOARD_LIMIT; i++) {
            for (int j = 0; j < BOARD_LIMIT; j++) {
                result[index++] = "%d,%d;0,false".formatted(i, j);
            }
        }
        return result;
    }
