...
import com.compass.partidos.controller.dto.PartidoDto;
import com.compass.partidos.controller.form.PartidoForm;
import com.compass.partidos.modelo.Partido;
import com.compass.partidos.repository.PartidoRepository;
...
public class PartidoController {
	...
	public List<PartidoDto> listaPartido(String ideologia){
		if(ideologia == null) {
	...
	@Transactional
	@CacheEvict(value = "listaDePartidos", allEntries = true)
	public ResponseEntity<PartidoDto> atualizar(@PathVariable Long id, @RequestBody @Valid PartidoForm partidoForm) {
		Optional<Partido> partidoOptional = partidoRepository.findById(id);
		if (partidoOptional.isPresent()) {
			Partido partido = partidoForm.atualizar(id, partidoForm);
			partidoRepository.save(partido);
